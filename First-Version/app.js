...
document.getElementById("exportJson").onclick = () => {
  ...
  if (multiMessageMode && messageQueue.length > 0) {
    ...
    if (!payload) {
      alert("⚠️ Nenhuma mensagem para exportar. Adicione conteúdo antes.");
      return;
    }
    exportData = {
      mode: "single",
  ...
  navigator.clipboard.writeText(json).then(() => {
    alert("JSON copiado para a área de transferência e baixado!");
  });
};
...
function renderAllEmbeds() {
  ...
  if (embeds.length === 0) {
    embedsContainer.innerHTML = '<p class="text-muted text-center">Nenhum embed adicionado.</p>';
    updatePreview();
    return;
  }
...
function renderNewEmbed(index) {
  ...
  const getEmbedPreview = () => {
    ...
    if (title && description) {
      ...
      return `<span class="text-muted">Embed ${index + 1}</span>`;
    }
  };
  ...
  embedCard.innerHTML = `
    <div class="card-header d-flex justify-content-between align-items-center">
      <div class="d-flex align-items-center flex-grow-1 min-width-0">
        <button class="btn btn-sm btn-light me-2 drag-handle" title="Arrastar">
          <i class="bi bi-arrows-move"></i>
        </button>
        <button class="btn btn-sm btn-outline-secondary me-2 embed-collapse-btn" type="button" data-bs-toggle="collapse" data-bs-target="#embedBody${index}" aria-expanded="true" title="Expandir/Colapsar">
      ...
      <div>
        <button class="btn btn-sm btn-outline-secondary me-1" onclick="moveEmbedUp(${index})" ${index === 0 ? 'disabled' : ''} title="Mover para cima">
          <i class="bi bi-arrow-up"></i>
        </button>
        <button class="btn btn-sm btn-outline-secondary me-1" onclick="moveEmbedDown(${index})" ${index === embeds.length - 1 ? 'disabled' : ''} title="Mover para baixo">
    ...
    <div id="embedBody${index}" class="collapse show">
      ...
      <div class="mb-3">
        <label class="form-label fw-bold">URL do Título</label>
        <input class="form-control" 
          placeholder="https://..."
          value="${embed.url}"
          data-embed="${index}"
      ...
      <div class="accordion mb-3" id="accordion${index}">
        ...
        <div class="accordion-item">
          ...
          <div id="author${index}" class="accordion-collapse collapse" data-bs-parent="#accordion${index}">
            <div class="accordion-body">
              <div class="mb-2">
                <input class="form-control" placeholder="Nome do autor"
                  value="${embed.authorName}"
                  data-embed="${index}"
                  data-field="authorName">
              </div>
              <div class="mb-2">
                <input class="form-control" placeholder="URL do autor"
                  value="${embed.authorUrl}"
                  data-embed="${index}"
                  data-field="authorUrl">
              </div>
              <div>
                <input class="form-control" placeholder="Icon URL do autor"
        ...
        <div class="accordion-item">
          ...
          <div id="footer${index}" class="accordion-collapse collapse" data-bs-parent="#accordion${index}">
            <div class="accordion-body">
              <div class="mb-2">
                <input class="form-control" placeholder="Texto do footer"
                  value="${embed.footerText}"
                  data-embed="${index}"
                  data-field="footerText">
              </div>
              <div class="mb-2">
                <input class="form-control" placeholder="Icon URL do footer"
              ...
              <div class="form-check">
                ...
                <label class="form-check-label">Incluir timestamp</label>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="accordion-item">
          <h2 class="accordion-header">
            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#fields${index}">
              <i class="bi bi-list-ul"></i> Fields (<span id="fieldCount${index}">${embed.fields.length}</span>)
            </button>
          </h2>
          <div id="fields${index}" class="accordion-collapse collapse" data-bs-parent="#accordion${index}">
            <div class="accordion-body">
              <button class="btn btn-sm btn-outline-primary mb-2" data-action="add-field" data-index="${index}">
                <i class="bi bi-plus"></i> Adicionar Field
              </button>
...
function renderFieldsHTML(embed, embedIndex) {
  if (embed.fields.length === 0) {
    return '<p class="text-muted small">Nenhum field adicionado.</p>';
  }
    ...
    <div class="card mb-2" id="field-${embedIndex}-${fieldIndex}" data-embed="${embedIndex}" data-field-index="${fieldIndex}">
      <div class="card-body">
        <div class="d-flex justify-content-between mb-2">
          ...
          <div>
            <button class="btn btn-sm btn-outline-secondary" onclick="moveFieldUp(${embedIndex}, ${fieldIndex})" ${fieldIndex === 0 ? 'disabled' : ''} title="Mover para cima">
              <i class="bi bi-arrow-up"></i>
            </button>
            <button class="btn btn-sm btn-outline-secondary" onclick="moveFieldDown(${embedIndex}, ${fieldIndex})" ${fieldIndex === embed.fields.length - 1 ? 'disabled' : ''} title="Mover para baixo">
              <i class="bi bi-arrow-down"></i>
            </button>
        ...
        <div class="mb-2">
          <input class="form-control form-control-sm" placeholder="Nome do field"
            value="${f.name}"
        ...
        <div class="mb-2">
          <textarea class="form-control form-control-sm" rows="2" placeholder="Valor do field"
            data-embed="${embedIndex}"
            data-field-index="${fieldIndex}"
...
function attachEmbedListeners(embedCard, embedIndex) {
  ...
  const updateHeaderPreview = () => {
    ...
    if (title && description) {
      ...
      previewHTML = `<span class="text-muted">Embed ${embedIndex + 1}</span>`;
    }
...
function removeEmbed(index) {
  if (confirm('Tem certeza que deseja remover este embed?')) {
    ...
    preview.innerHTML = `
      <discord-messages ${isLightTheme ? 'light-theme' : ''}>
        <discord-message bot author="${escapeHtml(username)}" avatar="${avatar || 'blue'}">
          <em style="opacity: 0.6;">Preview da mensagem aparecerá aqui...</em>
        </discord-message>
      </discord-messages>
    `;
    return;
...
function buildPayload(forExport = false) {
  ...
  if (!content && validEmbeds.length === 0 && !forExport) {
    alert("Mensagem vazia. Adicione conteúdo ou pelo menos um embed com dados.");
    return null;
  }
...
function toggleMultiMessageMode() {
  ...
  if (multiMessageMode) {
    ...
    sendBtn.innerHTML = '<i class="bi bi-send"></i> Adicionar ao Grupo';
    ...
    sendBtn.innerHTML = '<i class="bi bi-send"></i> Enviar Webhook';
  ...
  if (wasEditing) {
    showFeedback("Edição cancelada ao alternar modo", "warning");
  }
}
...
function addToQueue() {
  ...
  if (!hasContent && !hasEmbeds) {
    alert("⚠️ Adicione conteúdo ou embeds antes de adicionar ao grupo");
    return;
  }
  ...
  if (editingMessageIndex !== null) {
    ...
    feedbackText = '<i class="bi bi-check-lg"></i> Edição Salva!';
    editingMessageIndex = null;
    updateEditingUI();
  } else {
    // Adiciona nova mensagem
    messageQueue.push(messageData);
    feedbackText = '<i class="bi bi-check-lg"></i> Adicionado!';
  }
...
function updateQueueUI() {
  ...
  if (messageQueue.length === 0) {
    container.innerHTML = '<p class="text-muted text-center mb-0">Nenhuma mensagem no grupo</p>';
    return;
  }
  ...
  container.innerHTML = messageQueue.map((msg, index) => {
    ...
    const editingBadge = isEditing ? '<span class="badge bg-warning text-dark ms-2"><i class="bi bi-pencil-fill"></i> Editando</span>' : '';
    
    return `
      <div class="queue-item card mb-2${editingClass}" data-index="${index}" draggable="false">
        <div class="card-body p-2 d-flex align-items-center gap-2">
          <button class="btn btn-sm btn-light queue-drag-handle" title="Arrastar para reordenar">
            <i class="bi bi-grip-vertical"></i>
          </button>
          <div class="flex-grow-1 min-width-0">
            <div class="d-flex align-items-center mb-1">
              <span class="badge bg-primary me-2">#${index + 1}</span>
              ${msg.username ? `<strong class="text-truncate">${escapeHtml(msg.username)}</strong>` : '<span class="text-muted">Sem username</span>'}
              ${editingBadge}
            </div>
          ...
          <div class="d-flex gap-1">
            <div class="btn-group btn-group-sm">
              <button class="btn btn-outline-secondary" onclick="moveQueueUp(${index})" ${isFirst ? 'disabled' : ''} title="Mover para cima">
                <i class="bi bi-arrow-up"></i>
              </button>
              <button class="btn btn-outline-secondary" onclick="moveQueueDown(${index})" ${isLast ? 'disabled' : ''} title="Mover para baixo">
                <i class="bi bi-arrow-down"></i>
              </button>
            </div>
            <div class="btn-group btn-group-sm">
              <button class="btn btn-outline-primary" onclick="loadFromQueue(${index})" title="Editar" ${isEditing ? 'disabled' : ''}>
                <i class="bi bi-pencil"></i>
              </button>
              <button class="btn btn-outline-info" onclick="duplicateInQueue(${index})" title="Duplicar">
                <i class="bi bi-files"></i>
              </button>
              <button class="btn btn-outline-danger" onclick="removeFromQueue(${index})" title="Remover">
...
function getMessagePreview(msg) {
  ...
  if (msg.embeds && msg.embeds.length > 0) {
    parts.push(`${msg.embeds.length} embed(s)`);
  }
  
  return parts.join(' • ') || 'Mensagem vazia';
}
...
function clearQueue() {
  ...
  if (confirm(`⚠️ Tem certeza que deseja limpar todas as ${messageQueue.length} mensagens do grupo?`)) {
    messageQueue = [];
...
function updateEditingUI() {
  ...
  if (editingMessageIndex !== null) {
    // Modo edição ativo
    sendBtn.innerHTML = '<i class="bi bi-save"></i> Salvar Edição';
    
    // Transforma o botão de adicionar em cancelar edição (botão pequeno no header)
    if (addToQueueBtn) {
      addToQueueBtn.innerHTML = '<i class="bi bi-x-lg"></i> Cancelar Edição';
    ...
    if (addToQueueBtn) {
      addToQueueBtn.innerHTML = '<i class="bi bi-plus-lg"></i> Adicionar ao Grupo';
...
function cancelEdit() {
  ...
  if ((hasContent || hasEmbeds) && !confirm("⚠️ Descartar alterações?")) {
    return;
  }
...
async function sendAllMessages() {
  ...
  if (!url) {
    alert("⚠️ Webhook URL é obrigatória");
    return;
  }
  
  if (messageQueue.length === 0) {
    alert("⚠️ Nenhuma mensagem no grupo");
    return;
  }
  ...
  for (let i = 0; i < messageQueue.length; i++) {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        failed++;
        errors.push(`Mensagem #${i + 1}: ${data.error}`);
      }
  ...
  // Relatório final
  let report = `✅ ${sent} mensagem(ns) enviada(s) com sucesso`;
  if (failed > 0) {
    report += `\n❌ ${failed} falhou(ram)`;
    if (errors.length > 0) {
...
async function sendWebhook() {
  ...
  if (!url) return alert("Webhook URL é obrigatória");
...
function importSingleMessage(messageData) {
  if (multiMessageMode) {
    ...
    const action = confirm("Você está no modo múltiplas mensagens.\n\n✅ OK = Adicionar ao grupo\n❌ Cancelar = Carregar no editor");
    ...
    if (action) {
      ...
      updateQueueUI();
      alert("Mensagem adicionada ao grupo!");
    } else {
      // Carrega no editor
      loadFromJson(messageData);
      alert("Mensagem carregada no editor!");
    }
  } else {
...
function importMultipleMessages(messagesArray) {
  if (!Array.isArray(messagesArray) || messagesArray.length === 0) {
    alert("⚠️ Nenhuma mensagem encontrada no arquivo.");
    return;
  }
  
  if (!multiMessageMode) {
    // Está no modo único: perguntar se quer ativar modo múltiplo
    const switchMode = confirm(`Você importou um grupo com ${messagesArray.length} mensagem(ns).\n\n✅ OK = Ativar modo múltiplo e importar\n❌ Cancelar = Carregar apenas a primeira mensagem`);
    
    if (switchMode) {
      // Ativa modo múltiplo
      if (!ENABLE_MULTI_MESSAGE_MODE) {
        alert("⚠️ O modo múltiplas mensagens está desabilitado na configuração.");
        return;
      }
      ...
      alert("Primeira mensagem do grupo importada!");
      return;
    }
  }
  ...
  let action = "replace";
  if (messageQueue.length > 0) {
    action = confirm(`Você já tem ${messageQueue.length} mensagem(ns) no grupo.\n\n✅ OK = Substituir tudo\n❌ Cancelar = Adicionar ao grupo existente`) ? "replace" : "add";
  }
  ...
  updateQueueUI();
  alert(`${messagesArray.length} mensagem(ns) ${action === "replace" ? "importada(s)" : "adicionada(s) ao grupo"}!`);
}
